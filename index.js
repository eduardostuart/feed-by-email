...
const { isYesterdayOrToday } = require('./utils')
...
async function todayFeedItems () {
  ...
  return (await Promise.allSettled(promises))
    ...
    .map(response => {
      ...
      if (!isYesterdayOrToday(new Date(feed.lastBuildDate))) {
        return undefined
      }

      const items = feed.items.filter(feedItem => isYesterdayOrToday(new Date(feedItem.isoDate)))
